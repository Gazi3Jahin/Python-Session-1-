...
def check_temperatures(temperatures, threshold):
    
    if any(temp > threshold for temp in temperatures):
        print("Warning: At least one temperature exceeds the threshold of", threshold)

temperature_readings = [25.6, 27.8, 30.5, 24.9, 28.3]
threshold = 28.0
check_temperatures(temperature_readings, threshold)
...
# Assuming you have a function to send emails called send_email()
...
def check_stock(products, threshold):
    ...
    for product, stock in products.items():
        if stock < threshold:
            print(f"Restock needed for {product}. Current stock level: {stock}")
...
check_stock(product_stock, restock_threshold)
