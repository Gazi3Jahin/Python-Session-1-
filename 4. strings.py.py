...
# Bio data string
bio_data = "Name: Jahin, Age: 20, Country: Bangladesh"

name_start = bio_data.find("Name:") + len("Name:")
name_end = bio_data.find(",", name_start)
name = bio_data[name_start:name_end].strip()

age_start = bio_data.find("Age:") + len("Age:")
age_end = bio_data.find(",", age_start)
age = bio_data[age_start:age_end].strip()

country_start = bio_data.find("Country:") + len("Country:")
country = bio_data[country_start:].strip()
...
formatted_string = "Name: {name}, Age: {age}, Country: {country}".format(**person_data)
